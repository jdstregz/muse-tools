...
const playlist_tools_v2 = require("./playlists/playlist-tools-v2");
const playlist_features = require("./playlists/playlist-features");
...
app.get("/login", function(req, res) {
  ...
  var scope =
    "user-read-private user-read-playback-state streaming user-read-birthdate user-read-email user-read-email playlist-read-private playlist-modify-private playlist-read-collaborative playlist-modify-public user-library-read";
  res.redirect(
...
app.get("/recentlyAdded", function(req, res) {
  ...
  playlist_tools.createRecentlyAddedPlaylist(
    access_token,
    excludedPlaylists,
    playlistName,
    days,
...
app.get("/combinePlaylists", async function(req, res) {
  var access_token = req.query.access_token;
  var playlistName = req.query.playlistName;
  var includedPlaylists = req.query.includedPlaylists.split(",");

  console.log(includedPlaylists);

  const results = await playlist_features.combinePlaylists(
    access_token,
    includedPlaylists,
    playlistName
  );
...
app.get("/recentlyAddedLibrary", function(req, res) {
  ...
  playlist_tools.createRecentlyAddedLibraryPlaylist(
    access_token,
    playlistName,
    days,
    function(data) {
